...
if (categoryList) {
  ...
  categories.forEach((cate) => {
    categoryList.insertAdjacentHTML(
      "beforeend",
      `<div class="category-item"><h3>${cate.name}</h3></div>`,
    );
  });
}
...
if (featuredList) {
  const resFeatured = await fetch(
    "http://localhost:3000/products?sale_gte=1&_sort=sale&_order=desc&_limit=4",
  );
  const featuredProducts: Product[] = await resFeatured.json();
...
if (newList) {
  const resNew = await fetch(
    "http://localhost:3000/products?_sort=id&_order=desc&_limit=4",
  );
  const newProducts: Product[] = await resNew.json();
...
if (detailDiv) {
  ...
  if (id) {
    ...
    const productDetail: Product = await resDetail.json();
    ...
    const stockInfo = document.getElementById("stock-info") as HTMLElement;
    ...
    const relatedRes = await fetch(
      `http://localhost:3000/products?categoryId=${productDetail.categoryId}&id_ne=${id}&_limit=4`,
    );
    const relatedProducts: Product[] = await relatedRes.json();
...
if (productList && categoryFilter) {
  ...
  async function loadProducts(categoryId: string | null = null) {
    try {
...
const toggleBtn = chatbot.querySelector(".chatbot-toggle") as HTMLElement;
const chatWindow = chatbot.querySelector(".chatbot-window") as HTMLElement;
const closeBtn = chatbot.querySelector(".chatbot-close") as HTMLElement;
const input = chatbot.querySelector("input") as HTMLInputElement;
const sendBtn = chatbot.querySelector("button") as HTMLElement;
const messages = chatbot.querySelector(".chatbot-messages") as HTMLElement;
...
if (cartList) {
  ...
  cartList.addEventListener("click", async (e) => {
    ...
    if (target.classList.contains("increase")) {
      ...
      const qtySpan = cartItem.querySelector(".qty") as HTMLElement;
