const productTable = document.getElementById("product-body");
...
window.updateVariant = (index, field, value) => {
...
function renderStats() {
    try {
        ...
        const totalStock = productsData.reduce((sum, p) => sum + (p.variants?.reduce((s, v) => s + (v.stock || 0), 0) || 0), 0);
        const totalCategories = categoryData.length;
        const totalUsers = usersData.length;
        const lowStock = productsData.filter((p) => (p.variants?.reduce((s, v) => s + (v.stock || 0), 0) || 0) < 5).length;
...
if (productForm) {
    productForm.addEventListener("submit", async (event) => {
        ...
        const categoryInput = document.getElementById("product-categoryId");
        // Parse images
...
window.editProduct = async (id) => {
    try {
        ...
        const imagesInput = document.getElementById("product-images");
        const categoryInput = document.getElementById("product-categoryId");
        const formTitle = document.querySelector("#product-form h2");
